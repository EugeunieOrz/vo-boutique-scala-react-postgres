...
//  creditCardEncryptionService: CreditCardEncryptionService,
  ...
  private def purchaseTheItems1(
    ...
    customer.setId(data.userID.toString.substring(0, 20))
    ...
    val newOrder = Order(
      id = shoppingBag.id,
      dateTime = Some(clock.instant()),
      countryByIP = Some(CountryAtPurchase(data.countryByIP)),
      shoppingBag = Some(ShoppingBag(
        id = shoppingBag.id,
  ...
  /**  val firstDecrypted = creditCardEncryptionService.decrypt(
      card.cardNumber.cardNumber,
      card.cardNumber.nonce
    ) */

//    if (firstDecrypted != "Could not decrypt message.") {
      //
      val paymentType: PaymentType = new PaymentType()
      val creditCard: CreditCardType = new CreditCardType()

  //    val cardNum = firstDecrypted + card.cardNumber.lastDigits
...
/**    } else {
      Future.successful(BadRequest(
        ApiResponse("checkout.card.decryption.failed", Messages("checkout.card.decryption.failed"))
      ))
    } */
...
/**    } else {
  ...
  private def updateItemsInStock(addedItem: Item): Future[Unit] = {
    itemService.retrieve(addedItem.id).map { item =>
