    ...
    SignUpForm.form.bindFromRequest.fold(
      ...
      data => {
        ...
        val user = User(
          ...
          ageLimit = AgeLimit(data.ageLimit),
          dateOfBirth = None,
          passwordSurvey = None,
          loginAttempts = None,
          addressBook = None,
        ...
        if (data.newsletterFashion || data.newsletterFineJewelry ||
        data.newsletterHomeCollection) {
          newsletterService.retrieve(data.email).flatMap {
            case Some(n) => {
              newsletterService.save(n.copy(
                newsletterFashion = NewsletterFashion(data.newsletterFashion),
                newsletterFineJewelry = NewsletterFineJewelry(data.newsletterFineJewelry),
                newsletterHomeCollection = NewsletterHomeCollection(data.newsletterHomeCollection)
              ))
            ...
            case _ => {
              // new customer is not subscribed to newsletter
              val newsl = SubscribeToNewsletter(
                id = UUID.randomUUID(),
                email = data.email,
                lang = request.lang,
    ...
    request.body.validate[SignUpToShop].fold(
      ...
      data => {
        ...
        val user = User(
          ...
          ageLimit = AgeLimit(data.data.ageLimit),
          dateOfBirth = None,
          passwordSurvey = None,
          loginAttempts = None,
          addressBook = None,
    ...
    request.body.validate[CreateAccount].fold(
      ...
      data => {
        ...
        val user = User(
          ...
          cardWallet = None,
          newsletters = Newsletter(
            newsletterFashion = NewsletterFashion(false),
            newsletterFineJewelry = NewsletterFineJewelry(false),
            newsletterHomeCollection = NewsletterHomeCollection(false)
          ),
          wishlist = None,
          orders = None,
          shoppingBag = Some(data.shoppingBag),
