      ...
      email => {
        ...
        userService.retrieve(loginInfo).flatMap {
          case Some(user) =>
            ...
            authTokenService.create(user.userID, tokenExpiry).map { authToken =>
              println("creating link to change a password...")
              val url = jsRouter.absoluteURL("/auth/password/recovery/" + authToken.id)
              println(url)
              val lang = user.registration.lang
              mailerClient.send(Email(
                subject = messagesApi("auth.email.reset.password.subject")(lang),
                from = messagesApi("email.from")(lang),
                to = Seq(email),
                bodyText = Some(twirl.core.views.txt.emails.resetPassword(
                  user, url)(messagesApi, lang).body),
      ...
      Form("password" -> nonEmptyText).bindFromRequest.fold(
        ...
        password => userService.retrieve(authToken.userID).flatMap {
          case Some(user) if user.loginInfo.providerID == CredentialsProvider.ID =>
            ...
            userService.save(user.copy(accountStatus = Some("Active")))
            ...
            userService.retrieve(value.userID).flatMap {
              case Some(user) if user.loginInfo.providerID == CredentialsProvider.ID =>
                userService.save(user.copy(passwordSurvey =
                  Some(user.passwordSurvey.getOrElse(List.empty) :+ value.reason))).map { _ =>
